...
3. Synchronized blocks/methods - Prior to JDK 24, when a synchronized method or block is used the virtual thread is pinned to a platform thread and will not relinquish its control. This means it will hold the platform thread which can cause performance issues if there is IO happening inside the synchronized block. Use ReentrantLock instead of synchronized. Since JDK 24 (JEP 491, carried into the JDK 25 LTS), the JVM no longer pins the platform thread for synchronized blocks/methods, the virtual thread unmounts like it would for other blocking operations, so this pitfall mainly applies to JDK 21-23.
